...
public class ExceptionService {
	// 예외(Exception) : 소스 코드의 수정으로 해결이 가능한 오류
	...
	public int inputNumber() {
		...
		while(true) {
			try {
				...
				num = sc.nextInt(); // java.util.InputMismatchException : 잘못 입력
	...
	public void ex3() {
		...
		try {
			...
			// 관계 없는 예외는 순서 관계없이 catch문에 작성하면 된다
	...
	public void ex5() {
		...
		try {
			...
			// Exception은 모든 예외의 최상위 부모
			// -> Exception이 catch 매개변수로 작성되었다 == 예외 종류와 상관없이 모두 처리
	...
	public void methodC() throws IOException {
		...
		// 호출하는 곳에서 예외 처리를 받드시 해야한다!!
		// 단, UncheckedExcetion은 선택적으로 예외 처리할 수 있음
	...
	public void methodD() {
		...
		//   흔하게 발생할 수 있는 예외
		// --> 예외 처리 구문이 아니라 if문같은 단순 코드로 해결 가능
		// --> 예외 처리 구문을 받드시 작성 할 필요가 없음
